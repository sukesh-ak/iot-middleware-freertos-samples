...
sudo apt-get -y install gcc git wget make cmake libncurses-dev flex bison gperf python3-pip python3-setuptools python3-serial python3-cryptography python3-future python3-pyparsing ninja-build ccache libffi-dev libssl-dev dfu-util

printf "Testing your python version: \n"
python3 -V
#**Save Original Path - will need this when switching between versions**
printf "\nCreate base path variable\n"
...
printf "ESP Core dump directory in tools directory\n\n"

ln -sf $IDF_PATH/components/partition_table ./partition_table
printf "ESP Partition Table directory in tools directory\n\n"

ln -sf $IDF_PATH/components/app_update ./app_update
printf "App Update tool directory in tools directory\n\n"
